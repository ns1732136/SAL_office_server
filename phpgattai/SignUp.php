...
if (isset($_POST["signUp"])) {
	...
	$mail = $_POST["mail"];
        ...
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO SiyakusyoKaigoUserData(name, password, syozoku, mail, bunrui) VALUES (?, ?, ?, ?, ?)");

            $stmt->execute(array($username, password_hash($password, PASSWORD_DEFAULT),$syozoku ,$mail ,$bunrui));  // パスワードのハッシュ化を行う（今回は文字列のみなのでbindValue(変数の内容が変わらない)を使用せず、直接excuteに渡しても問題ない）
...
<script>
history.replaceState({location_replace: "Itiran.php"}, "")
history.pushState({}, "", "SignUp.php")
